...
struct CameraView {
    ...
    float   fovH;     // горизонтальный FOV (градусы) — legacy fallback
    float   fovV = 0.f; // ВЕРТИКАЛЬНЫЙ FOV (градусы) — из игры (FPManager). 0 => неизвестно.
    bool    valid;    // удалось ли собрать данные камеры
    bool    aiming = false; // игрок целится (ADS/scope) — FOV упал ниже базового
    // Полный базис камеры из УГЛОВ взгляда (kqP.x pitch / kqP.y yaw). Если
    // hasBasis, W2S делает корректную перспективную проекцию в пространстве
    // камеры вместо раздельных yaw/pitch (которые точны только у центра экрана).
    bool    hasBasis = false;
    Vector3 right{1,0,0};
    Vector3 up{0,1,0};
