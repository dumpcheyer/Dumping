...
bool ImGui_init() {
    ...
    verdana = io.Fonts->AddFontDefault(&font_cfg);
...
// Опциональный сбор статистики отрисовки меню (определён в main.cpp).
// Вызывается ПОСЛЕ ImGui::Render(): читает GetDrawData() уже собранного кадра.
extern void ox_uiCaptureFrameStats();

void drawEnd() {
    ImGui::Render();
    ox_uiCaptureFrameStats();
