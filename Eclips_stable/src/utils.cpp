...
ImVec2 w2s_angular(Vector3 target, const CameraView& cam, bool* visible) {
    ...
    if (cam.hasBasis) {
        ...
        // FOV: Unity Camera.fieldOfView = ВЕРТИКАЛЬНЫЙ. Читаем его напрямую из
        // игры (cam.fovV, из FPManager). Горизонтальный выводим: tanH=tanV*(W/H).
        // Это чинит «боксы уезжают к краям» и корректно масштабирует при ADS/зуме
        // (fovV меняется в реальном времени). Fallback на горизонтальный слайдер
        // только если fovV неизвестен (нет FPManager в этот кадр).
        float tanVb, tanHb;
        if (cam.fovV > 1.f) {
            tanVb = tanf(cam.fovV * 0.5f * DEG2RAD);
            tanHb = tanVb * (W / H);
        } else {
            float fovHb = cam.fovH > 1.f ? cam.fovH : 90.f;
            tanHb = tanf(fovHb * 0.5f * DEG2RAD);
            tanVb = tanHb * (H / W);
        }
        ...
        if (L) OXLOGT("  W2S(basis) v=(%.2f,%.2f,%.2f) -> экран=(%.1f,%.1f) on=%d [fovV=%.1f tanH=%.3f tanV=%.3f]",
                      vx, vy, vz, x, y, (int)onScreen, cam.fovV, tanHb, tanVb);
