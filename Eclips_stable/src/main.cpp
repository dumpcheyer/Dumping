...
static constexpr uint64_t OX_META_NAME_OFF_PLAYERMANAGER = 0x171ebc;  // "PlayerManager" (was 0x1704f0)
static constexpr uint64_t OX_META_NAME_OFF_BUILDINGPIECE = 0x10c433;  // "BuildingPiece" (was 0x10c22b)
static constexpr uint64_t OX_META_NAME_OFF_PLAYERVITALS  = 0x16aebf;  // "PlayerVitals"  (was 0x169546)
...
// Найденные автопоиском адреса слотов. Ноль = не найдены, работаем по
// захардкоженным константам (они верны для arm64-сборки 2026/07/24).
static uint64_t g_autoMcRva     = 0;
static uint64_t g_autoBaseRva   = 0;
static uint64_t g_autoHeaderRva = 0;
static uint64_t g_autoStrideRva = 0;
...
// ============================================================================
//  АВТОПОИСК СЛОТОВ MetadataCache
//
//  Пять констант ниже (MC/BASE/HEADER/STRIDE) — это адреса слотов в .bss
//  libil2cpp, куда рантайм записывает указатели при инициализации. В файле
//  там нули, поэтому найти их статически нельзя — только в рантайме, когда
//  игра уже загрузилась.
//
//  Раньше они добывались дизассемблированием ARM64 и вписывались руками после
//  каждого обновления. Здесь то же самое делается автоматически, и заодно
//  снимается привязка к архитектуре: на x86_64 (BlueStacks) адреса другие,
//  но СТРУКТУРА расположения слотов та же.
//
//  Опознаём по форме: идут четыре слота подряд с шагом 8/8/4, где
//    +0x00  указатель на MetadataCache
//    +0x10  указатель на блоб метаданных (там лежит магия 0xFAB11BAF)
//    +0x18  указатель на заголовок
//    +0x24  stride (1, 2 или 4)
//  Магия в блобе — самый жёсткий признак, случайно не совпадёт.
// ============================================================================
static bool ox_autoFindMetadataSlots() {
    if (!il2cpp_base) return false;

    // Диапазон поиска: .bss обычно в последней трети образа. Берём широкое
    // окно и идём с шагом 8 — слоты выровнены по указателю.
    const uint64_t SCAN_FROM = 0x0B000000ULL;
    const uint64_t SCAN_TO   = 0x0D000000ULL;
    const uint64_t STEP      = 8;

    for (uint64_t off = SCAN_FROM; off < SCAN_TO; off += STEP) {
        uint64_t metaPtr = rpm<uint64_t>(il2cpp_base + off + 0x10);
        if (!ox_isPtr(metaPtr)) continue;

        // Проверка №1: по этому указателю обязана лежать магия метаданных.
        uint32_t magic = rpm<uint32_t>(metaPtr);
        if (magic != 0xFAB11BAF) continue;

        // Проверка №2: рядом заголовок и вменяемый stride.
        uint64_t hdrPtr = rpm<uint64_t>(il2cpp_base + off + 0x18);
        uint32_t stride = rpm<uint32_t>(il2cpp_base + off + 0x24);
        if (!ox_isPtr(hdrPtr) || stride == 0 || stride > 4) continue;

        // Проверка №3: сам MetadataCache — валидный указатель.
        uint64_t mc = rpm<uint64_t>(il2cpp_base + off);
        if (!ox_isPtr(mc)) continue;

        g_autoMcRva     = off;
        g_autoBaseRva   = off + 0x10;
        g_autoHeaderRva = off + 0x18;
        g_autoStrideRva = off + 0x24;
        OXLOGI("[auto] слоты MetadataCache найдены: mc=+0x%llX base=+0x%llX "
               "hdr=+0x%llX stride=+0x%llX (значение stride=%u)",
               (unsigned long long)g_autoMcRva, (unsigned long long)g_autoBaseRva,
               (unsigned long long)g_autoHeaderRva, (unsigned long long)g_autoStrideRva,
               stride);
        return true;
    }
    OXLOGW("[auto] слоты MetadataCache не найдены сканом — падаю на константы");
    return false;
}
...
static bool ox_cacheMetadataChain() {
    ...
    // Один раз пытаемся найти слоты автоматически. Если вышло — дальше
    // используются найденные адреса, а не захардкоженные.
    static bool s_autoTried = false;
    if (!s_autoTried) {
        s_autoTried = true;
        ox_autoFindMetadataSlots();
    }
    if (g_metadataBase && g_typesTableAddr && g_typeStride && g_sTypeInfoTable) return true;

    const uint64_t rvaBase   = g_autoBaseRva   ? g_autoBaseRva   : OX_META_BASE_PTR_RVA;
    const uint64_t rvaHeader = g_autoHeaderRva ? g_autoHeaderRva : OX_META_HEADER_PTR_RVA;
    const uint64_t rvaStride = g_autoStrideRva ? g_autoStrideRva : OX_META_STRIDE_RVA;
    const uint64_t rvaMc     = g_autoMcRva     ? g_autoMcRva     : OX_META_MC_RVA;

    uint64_t meta   = rpm<uint64_t>(il2cpp_base + rvaBase);
    uint64_t hdr    = rpm<uint64_t>(il2cpp_base + rvaHeader);
    uint32_t stride = rpm<uint32_t>(il2cpp_base + rvaStride);
    ...
    uint64_t mc = rpm<uint64_t>(il2cpp_base + rvaMc);
    if (!ox_isPtr(mc)) {
...
static bool ox_fastSeedTypeInfoFromHeader() {
    ...
    // Адрес таблицы: сначала пробуем отсчитать от автонайденного слота
    // MetadataCache — мнемоника «ctx + 0x48» держится во всех проверенных
    // сборках. Это снимает привязку и к обновлениям, и к архитектуре: на
    // x86_64 (BlueStacks) абсолютные адреса другие, а смещение то же.
    uint64_t tableRva = g_autoMcRva ? (g_autoMcRva + 0x48) : OX_S_TYPEINFO_TABLE_RVA;
    uint64_t table = rpm<uint64_t>(il2cpp_base + tableRva);

    // Если по вычисленному адресу пусто — пробуем захардкоженный, и наоборот.
    if (!ox_isPtr(table) && tableRva != OX_S_TYPEINFO_TABLE_RVA) {
        tableRva = OX_S_TYPEINFO_TABLE_RVA;
        table = rpm<uint64_t>(il2cpp_base + tableRva);
    }
    if (!ox_isPtr(table)) {
        OXLOGD("[fast-seed] s_TypeInfoTable slot @ RVA 0x%llx = 0x%llx (init ещё не прогнан?)",
               (unsigned long long)tableRva, (unsigned long long)table);
        return false;
    }
    ...
    if (!g_networkClientKlass) {
        // Тот же автонайденный адрес, что и выше — иначе на x86 эта ветка
        // читала бы по arm64-константе и молча ничего не находила.
        uint64_t tRva = g_autoMcRva ? (g_autoMcRva + 0x48) : OX_S_TYPEINFO_TABLE_RVA;
        uint64_t table = rpm<uint64_t>(il2cpp_base + tRva);
        if (!ox_isPtr(table) && tRva != OX_S_TYPEINFO_TABLE_RVA)
            table = rpm<uint64_t>(il2cpp_base + OX_S_TYPEINFO_TABLE_RVA);
        if (ox_isPtr(table)) {
                              ...
                              const ImVec2* lineOrigin, float uiScaleFix)
{
    ...
    ImU32 accent;
    if (espColorByVisibility && losBlocked)      accent = IM_COL32(80, 150, 255, 255);
    else if (espColorByVisibility)               accent = IM_COL32(60, 230, 110, 255);
...
static void oxTabAim() {
    ...
    Toggle(L_ADS, &aimOnlyADS, L_ADSH);
