...
static OxLookWatch g_lookWatch;

// Возвращает true если углы этого PM «мертвы» (не менялись дольше stallSec).
static bool ox_lookFrozen(uint64_t pm, double nowSec, float stallSec = 2.5f) {
    ...
    if (g_lookWatch.pm != pm || !g_lookWatch.seeded) {
        g_lookWatch.pm = pm; g_lookWatch.pitch = p; g_lookWatch.yaw = y;
        g_lookWatch.lastChange = nowSec; g_lookWatch.seeded = true;
        return false;                      // только начали наблюдать
    }
    // Порог 1e-4 градуса — реальные углы всегда дрожат сильнее.
    if (fabsf(p - g_lookWatch.pitch) > 1e-4f || fabsf(y - g_lookWatch.yaw) > 1e-4f) {
        g_lookWatch.pitch = p; g_lookWatch.yaw = y;
        g_lookWatch.lastChange = nowSec;
        return false;
    }
    return (nowSec - g_lookWatch.lastChange) > (double)stallSec;
}

// Сбросить наблюдение (при смене локального игрока).
static void ox_lookWatchReset() { g_lookWatch = OxLookWatch{}; }
...
static CameraView ox_getCamera(uint64_t elems, int count, bool full) {
    ...
    for (int ci = 0; ci < nc; ci++) {
        ...
        // Если кандидатов несколько, отдаём предпочтение тому, чьи углы живут.
        // Единственного кандидата берём как есть — иначе после респавна, пока
        // новый PM ещё не пошевелился, ESP не запустится вообще.
        if (nc > 1 && player != g_localPlayer && ox_lookFrozen(player, nowSec, 1.2f))
            continue;
        if (full)
