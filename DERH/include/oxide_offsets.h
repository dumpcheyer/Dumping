...
// UPDATE 2026/07/25 (второй апдейт за сутки). libil2cpp md5
// 6b94d237d6319d1cf0090b09d8a8620e, metadata md5 10a92ff40089451e7f9a6628a9a306b7.
// TDI НЕ изменились, byval сдвинулись. Все 9 проверены:
// types[byval].klassIndex == TDI, kind=0x12 CLASS, byref=0, pinned=0.
static constexpr uint64_t IL2CPP_TYPES_RVA               = 0xB75E020; // 104776 entries (was 0xB77C750)
static constexpr int32_t  PLAYERMANAGER_TYPEINFO_TYPEIDX  = 61090;   // Oxide.PlayerManager (TDI 8357)   VERIFY OK (was 60998)
static constexpr int32_t  BUILDINGPIECE_TYPEINFO_TYPEIDX  = 46495;   // Oxide.Building.BuildingPiece (TDI 8633)   VERIFY OK (was 46457)
static constexpr int32_t  PLAYERVITALS_TYPEINFO_TYPEIDX   = 61154;   // Oxide.PlayerVitals (TDI 8030)   VERIFY OK (was 61062)
static constexpr int32_t  RAYCASTMANAGER_TYPEINFO_TYPEIDX = 62325;   // Oxide.RaycastManager (TDI 8155)   VERIFY OK (was 62232)
static constexpr int32_t  MOUSELOOK_TYPEINFO_TYPEIDX      = 58868;   // Oxide.MouseLook (TDI 8009)   VERIFY OK (was 58788)
static constexpr int32_t  CAMERA_TYPEINFO_TYPEIDX         = 46827;   // UnityEngine.Camera (TDI 13810)   VERIFY OK (was 46796)
static constexpr int32_t  ENTITYVITALS_TYPEINFO_TYPEIDX   = 50778;   // Oxide.EntityVitals (TDI 8019)   VERIFY OK (was 50770)
static constexpr int32_t  GENERICVITALS_TYPEINFO_TYPEIDX  = 52626;   // Oxide.GenericVitals (TDI 8021)   VERIFY OK (was 52632)
...
// ── Доп. поля PlayerManager для расширенного ESP (дамп 2026/07/25) ──────────
static constexpr uint64_t PM_NICKLABEL         = 0x130; // lH* nicklabel (UI-подпись игрока)
static constexpr uint64_t PM_PLAYER_FLAGS      = 0x250; // PlayerFlags (Sleeping/Spectating/Muted)
...
// nicklabel (класс lH): держит UI-тексты с ником и id игрока.
//   lH + 0x38 -> UnityEngine.UI.Text nickname
//   lH + 0x40 -> UnityEngine.UI.Text playerId
// UnityEngine.UI.Text + 0xE0 -> String m_Text
static constexpr uint64_t NICKLABEL_NICKNAME   = 0x38;
static constexpr uint64_t NICKLABEL_PLAYERID   = 0x40;
static constexpr uint64_t UITEXT_M_TEXT        = 0xE0;

// ── Оружие в руках: PM -> FPManager -> FPObject -> Item -> ItemData -> имя ──
//   PM         + 0x90 -> FPManager
//   FPManager  + 0x50 -> FPObject _currentWeapon
//   FPObject   + 0x40 -> Item     (_cSb_k__BackingField)
//   Item       + 0x28 -> ItemData (_cuA_k__BackingField)
//   ItemData   + 0x20 -> String   m_ShortName   (короткое имя, для ESP-подписи)
//   ItemData   + 0x18 -> String   m_Name
static constexpr uint64_t FPOBJ_ITEM           = 0x40; // Item*
static constexpr uint64_t ITEM_DATA            = 0x28; // ItemData*
...
// ── Броня: GenericVitals + 0x90 -> ProtectionValues ────────────────────────
static constexpr uint64_t VITALS_PROTECTION    = 0x90; // ProtectionValues*
...
//   +0x20 List<TeamMember> mYK          — СОСТАВ команды
// TeamMember (TypeDefIndex 549):
//   +0x10 string id     — userID участника (совпадает с PM.userID @0x278)
//   +0x18 string clanTag
//   +0x20 string nick
//   +0x28 int    lvl
//   +0x2C uint   netId
// Прямая проверка «этот игрок — мой сокомандник»: берём pf локального игрока,
// идём по mYK и сверяем TeamMember.id с PM.userID цели. Не зависит от того,
// заполнены строковые поля команды или нет.
static constexpr uint64_t PF_TEAM_MEMBERS      = 0x20; // List<TeamMember> mYK
static constexpr uint64_t TEAMMEMBER_ID        = 0x10; // string id (== PM.userID)  // pf team (data object)
...
// ── Скелет: цепочка от PlayerManager до костей модели ──────────────────────
// PM.animator (0x190) -> Animator; но нам нужен PlayerModelInfo, который
// висит на CharacterAnimation. Путь (все оффсеты из il2cpp.h дампа 2026/07/24):
//   PM + PM_ANIMATOR(0x190)              -> UnityEngine.Animator
//   PM + PM_CHARACTER_MODEL(0x150)       -> GameObject модели
// PlayerModelInfo (HyperHug...Player.PlayerModelInfo):
//   +0x20 head          Transform    <- голова
//   +0x28 rightWeaponHolder Transform <- правая кисть
//   +0x30 leftWeaponHolder  Transform <- левая кисть
//   +0x40 body          Transform    <- корпус (таз/грудь)
//   +0x60 characterAnimation -> CharacterAnimation
// CharacterAnimation:
//   +0x30 playerModelInfo -> обратно на PlayerModelInfo
//   +0x38 ragdoll         -> Ragdoll (у него +0x88 m_Bones — массив всех костей)
...
static constexpr uint64_t PMI_CHAR_ANIM       = 0x60; // CharacterAnimation*
static constexpr uint64_t CHARANIM_MODELINFO  = 0x30; // PlayerModelInfo*
static constexpr uint64_t CHARANIM_RAGDOLL    = 0x38; // Ragdoll*
static constexpr uint64_t RAGDOLL_PELVIS      = 0x20; // Rigidbody m_Pelvis
static constexpr uint64_t RAGDOLL_BONES       = 0x88; // List<Transform> m_Bones
...
// ox_readPos возвращает lastTickPosition (server-authoritative anti-cheat поле).
// Уровень этой точки зависит от того где у контроллера pivot — ноги, центр
// капсулы или голова. На дампе 2026/07/24 бокс вставал ВЫШЕ цели, значит pos
// не на ногах. Поэтому вертикальная привязка вынесена в живой слайдер
// espBoxYOffset (меню ESP -> BOX -> Vertical offset), а эти две константы
// задают только ВЫСОТУ бокса относительно привязки.
static constexpr float BOX_FOOT     = 0.0f;   // низ бокса = pos + yOffset
static constexpr float BOX_HEAD     = 1.85f;  // верх бокса = pos + yOffset + BOX_HEAD
