...
void Layout_tick_UI() {
    ...
    if (openK > 0.015f) {
        ...
        ImVec2 sz(portrait ? ds.x * 0.92f : ds.x * 0.94f, ds.y * 0.88f);
        ...
        if (!portrait) {
            ...
            if (optW < minOpt) {               // деградация на узких экранах:
                ...
                float give = prevW - 190.0f * s; if (give < 0.f) give = 0.f;
                float d = need < give ? need : give;
                prevW -= d; need -= d;
                if (need > 0.f) {
                    give = navW - 150.0f * s; if (give < 0.f) give = 0.f;
            ...
            // ---- PREVIEW: центральная колонка, без скролла, центр по вертикали
            {
                ...
                float paneH = availH < Px(430.0f) ? availH : Px(430.0f);
